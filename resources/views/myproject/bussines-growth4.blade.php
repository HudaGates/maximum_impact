...
<form method="POST" action="{{ route('business-growth.step4.store') }}">
    @csrf
    <div class="growth-container">
        <h4><strong>Write your business’s team development target for the next 6 months?</strong></h4>
        
        {{-- Menampilkan bulan yang sedang diisi secara dinamis --}}
        <p><strong>Month {{ $month }}:</strong> <span style="float: right;">4/6</span></p>

        <div style="position: relative;">
            {{-- 2. NAMA INPUT DIUBAH MENJADI 'team_dev_target' --}}
            {{-- 3. NILAI TEXTAREA DIISI DENGAN DATA YANG SUDAH ADA --}}
            <textarea name="team_dev_target" required>{{ old('team_dev_target', $data->team_dev_target ?? '') }}</textarea>
        ...
        <div class="btn-container">
            ...
            <a href="{{ route('business-growth.step3.show', ['month' => $month]) }}" class="btn">Back</a>
