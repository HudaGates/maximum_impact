...
<form method="POST" action="{{ route('business-growth.step2.store') }}">
    @csrf
    <div class="growth-container">
        <h4><strong>Write the monthly revenue target in the next 6 months?</strong></h4>
        <p><strong>Month {{ $month }}:</strong> <span style="float: right;">2/6</span></p>
        <div style="position: relative;">
            {{-- 2. NAMA INPUT DIUBAH MENJADI 'revenue_target' --}}
            {{-- 3. NILAI TEXTAREA DIISI DENGAN DATA YANG SUDAH ADA --}}
            <textarea name="revenue_target" required>{{ old('revenue_target', $data->revenue_target ?? '') }}</textarea>
        ...
        <div class="btn-container">
            ...
            <a href="{{ route('business-growth.step1.show', ['month' => $month]) }}" class="btn">Back</a>
