...
<form method="POST" action="{{ route('business-growth.step5.store') }}">
    @csrf
    <div class="growth-container">
        <h4><strong>Write your business’s social impact target for the next 6 months?</strong></h4>
        
        {{-- Menampilkan bulan yang sedang diisi secara dinamis --}}
        <p><strong>Month {{ $month }}:</strong> <span style="float: right;">5/6</span></p>

        <div style="position: relative;">
            {{-- 2. NAMA INPUT DIUBAH MENJADI 'social_impact_target' --}}
            {{-- 3. NILAI TEXTAREA DIISI DENGAN DATA YANG SUDAH ADA --}}
            <textarea name="social_impact_target" required>{{ old('social_impact_target', $data->social_impact_target ?? '') }}</textarea>
        ...
        <div class="btn-container">
            {{-- 5. Tombol back mengarah ke halaman sebelumnya dengan menyertakan bulan --}}
            <a href="{{ route('business-growth.step4.show', ['month' => $month]) }}" class="btn">Back</a>
